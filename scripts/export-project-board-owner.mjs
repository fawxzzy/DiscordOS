...
const REGISTRY_PATH = "config/discordos-owner-work-registry.json";
const ATLAS_REGISTRY_PATH = "repos/DiscordOS/config/discordos-owner-work-registry.json";
const DEFAULT_OUTPUT_PATH = "exports/discordos.project-board.owner-export.v1.json";
const BOARD_ID = "discordos:project-feedback:discordos";
...
function mapItem(item, generatedAt) {
  ...
  if (!mapping) throw new Error(`unsupported non-complete registry status for ${item.id}: ${JSON.stringify(item.status)}`);
  if (typeof item.id !== "string" || !/^DOS-[A-Z0-9-]+$/.test(item.id)) throw new Error("work item id must use the DOS-* format");
  ...
  return {
    idempotency_key: `pbk_discordos_${normalizedId}_v1`,
    ...
    record: {
      ...
      extensions: { owner_status: item.status, owner_registry_schema_version: 1 }
    },
    source: {
      source_id: "discordos-owner-work-registry",
      source_ref: sourceRef,
    ...
    content: {
      ...
      evidence: uniqueSorted(stringArray(item, "evidence", false).map(atlasPath))
    },
...
export function buildProjectBoardOwnerExport(registry, registryBytes) {
  if (!registry || registry.schemaVersion !== 1 || registry.projectId !== "discordos") throw new Error("unexpected DiscordOS owner registry identity");
  if (registry.state !== "active") throw new Error("DiscordOS owner registry must declare state=active");
  if (!Array.isArray(registry.workItems) || registry.workItems.length === 0) throw new Error("registry.workItems must not be empty");
  const ids = registry.workItems.map((item) => item.id);
  if (new Set(ids).size !== ids.length) throw new Error("registry work item ids must be unique");
  ...
  return {
    ...
    export_id: `pbe_discordos_owner_registry_${digest.slice(0, 12)}`,
    project_id: "discordos",
    board_id: BOARD_ID,
    owner: "discordos",
    ...
    extensions: {
      ...
      owner_work_state: registry.state,
      atlas_candidates_reconciled: true,
...
export function runProjectBoardOwnerExport(argv, repoRoot = process.cwd()) {
  ...
  process.stdout.write(`discordos-project-board-owner-export: wrote ${DEFAULT_OUTPUT_PATH}\n`);
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.error(`discordos-project-board-owner-export: ${error.message}`);
    process.exitCode = 1;
  }
}
