...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The host-control trend alert delivery canary now proves routed alerts can pass delivery admission without sending; the next value is readback on the delivery decision.",
    does: "Confirms alert admission, route identity, and no-send/no-playback boundaries after the delivery canary evaluates.",
  },
  {
    ...
    why: "The provider queue interaction admission gate now admits only signed metadata-only queue behavior; the next value is readback on that admission state.",
    does: "Confirms custom id, provider track metadata, signature proof, and no-provider/no-playback boundaries after admission.",
  },
  {
    ...
    why: "Acknowledgement readback now confirms handled state and redaction; the next value is operator dashboard visibility for closed alert state.",
    does: "Summarizes handled acknowledgement readback, redaction status, and route identity without exposing actor ids or tokens.",
  },
  {
    ...
    why: "Rate-limit observability now shows allow/throttle decisions without user content; the next value is alerting only when throttling needs operator attention.",
    does: "Routes throttle attention from rate-limit observability while keeping user content hidden and mention safety intact.",
  },
  {
    ...
    why: "The scheduler alert delivery canary now proves alert admission while preserving reaction guards; the next value is readback on that delivery decision.",
    does: "Confirms alert admission, custom-reaction guard preservation, and readback requirements after the delivery canary evaluates.",
  },
];
