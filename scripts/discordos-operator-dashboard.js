...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The host-control rollup history now has a bounded persistence model; the next value is detecting repeated conflict or readback drift trends.",
    does: "Summarizes recent host-control history into operator-safe trend alerts without sending messages or controlling playback.",
  },
  {
    ...
    why: "The provider queue surface now has a guarded publish/readback plan; the next value is proving the button interaction maps back to queue metadata.",
    does: "Runs a signed button interaction readback against the published provider queue surface and verifies it queues metadata only.",
  },
  {
    ...
    why: "The alert runbook linkage is now bounded and redacted; the next value is an acknowledgement path so operators can close handled audit alerts.",
    does: "Adds no-slash acknowledgement metadata and readback for button-route audit alerts while preserving actor/token redaction.",
  },
  {
    ...
    why: "The non-testing live-readback path is double-guarded; the next value is preventing repeated user-facing responses from becoming noisy.",
    does: "Adds a bounded per-channel response delivery rate-limit read model with mention safety and no slash-command entrypoints.",
  },
  {
    ...
    why: "The scheduler now has a guarded apply decision; the next value is a compact view of repair attempts, skipped aligned cards, and readback results.",
    does: "Builds an operator rollup for scheduled reaction repairs so drift repair can be audited without opening every board card.",
  },
];
