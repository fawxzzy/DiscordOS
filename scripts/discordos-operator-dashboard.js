...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "Delivery readback now proves the alert decision and route identity without sending; the next value is dashboard visibility for that delivery state.",
    does: "Summarizes host-control trend alert admission, route identity, and no-send/no-playback/no-provider boundaries for operator scanning.",
  },
  {
    ...
    why: "Admission readback now confirms signed metadata-only queue behavior; the next value is dashboard visibility for provider queue safety.",
    does: "Summarizes admission state, track metadata visibility, signature proof, and no-provider/no-playback boundaries.",
  },
  {
    ...
    why: "The acknowledgement dashboard now shows closed alert state safely; the next value is bounded history for repeated acknowledgement patterns.",
    does: "Tracks recent acknowledgement dashboard states by route while preserving actor/token redaction and no-write boundaries.",
  },
  {
    ...
    why: "Rate-limit alerting now classifies throttle attention without sending; the next value is a delivery canary for that alert path.",
    does: "Confirms throttling alerts can be admitted as no-send operator attention while keeping user content hidden.",
  },
  {
    ...
    why: "Scheduler alert delivery readback now confirms admission and guard preservation; the next value is dashboard visibility for repair-alert delivery state.",
    does: "Summarizes scheduler alert admission, custom-reaction guard preservation, readback requirements, and no-send boundaries.",
  },
];
