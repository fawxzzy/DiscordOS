...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The guarded host-control storage canary now proves the rules can be modeled and read back; the next value is an operator view of persisted session state.",
    does: "Summarizes recent Music Sesh session, queue, vote, lock, and close state so operators can see whether host controls are healthy.",
  },
  {
    ...
    why: "Provider metadata can now become safe queue previews; the next step is letting users choose a preview with buttons instead of slash commands.",
    does: "Turns preview rows into button custom IDs and validates that selection queues metadata without provider playback.",
  },
  {
    ...
    why: "Button audits now have a dashboard; the next risk is missed failures when route execution starts drifting.",
    does: "Routes operator alerts for button route failure spikes, storage-write misses, and unexpected response types without exposing raw user IDs.",
  },
  {
    ...
    why: "A testing-surface delivery canary proves one controlled send; operators need visibility into delivery policy before user-facing expansion.",
    does: "Summarizes response-delivery admission, allowed-mention safety, target surface, and readback status for Music Sesh messages.",
  },
  {
    ...
    why: "Drift alerting can now identify mismatched board reactions; the next value is a guarded repair canary that fixes only admitted cards.",
    does: "Runs a no-surprise repair preview for drifted Music Sesh cards and applies reaction repair only under the existing double guard.",
  },
];
