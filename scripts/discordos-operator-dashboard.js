...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The rollup now summarizes repeated host-control delivery history; the next value is a dashboard that makes route and alert-level trends operator-scannable.",
    does: "Shows host-control trend alert delivery rollups without sending messages, controlling playback, or calling providers.",
  },
  {
    ...
    why: "Provider admission history alerting now classifies no-send attention; the next value is proving alert delivery admission without provider or playback execution.",
    does: "Canaries provider admission history alert delivery while preserving signature proof, metadata-only queue state, and no-send behavior.",
  },
  {
    ...
    why: "The acknowledgement alert delivery canary now proves no-send admission; the next value is readback that confirms the redacted alert path stays inspectable.",
    does: "Reads back acknowledgement alert delivery decisions without sending, writing storage, or exposing actor/token data.",
  },
  {
    ...
    why: "The rate-limit alert delivery dashboard now makes decisions scan-ready; the next value is bounded history for repeated rate-limit delivery states.",
    does: "Tracks recent rate-limit alert delivery dashboard decisions while keeping user content hidden and preserving no-send behavior.",
  },
  {
    ...
    why: "Scheduler alert delivery history alerting now classifies repeated guarded states; the next value is a canary for no-send alert delivery admission.",
    does: "Proves repair scheduler history alert delivery admission while preserving custom-reaction guards, readback requirements, and no-send behavior.",
  },
];
