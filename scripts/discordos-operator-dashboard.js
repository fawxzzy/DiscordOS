...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The delivery dashboard now makes alert admission and route boundaries scan-ready; the next value is bounded history for repeated delivery states.",
    does: "Tracks recent host-control trend alert delivery dashboard states without sending, playback control, or provider calls.",
  },
  {
    ...
    why: "The admission dashboard now summarizes metadata-only provider queue safety; the next value is bounded history for repeated admission states.",
    does: "Tracks recent provider queue admission dashboard states while preserving signature proof and no-provider/no-playback boundaries.",
  },
  {
    ...
    why: "Acknowledgement dashboard history now bounds repeated closed-alert records; the next value is alerting only when those patterns need operator attention.",
    does: "Classifies repeated acknowledgement history for no-send attention while preserving actor/token redaction and no-write boundaries.",
  },
  {
    ...
    why: "The rate-limit alert delivery canary now proves alert admission without Discord sends; the next value is readback on that delivery decision.",
    does: "Confirms rate-limit alert admission, no-send delivery boundaries, mention safety, and hidden user content after the canary evaluates.",
  },
  {
    ...
    why: "The scheduler alert delivery dashboard now exposes repair-alert state safely; the next value is bounded history for repeated repair-alert delivery states.",
    does: "Tracks recent scheduler alert delivery dashboard states while preserving custom-reaction guards, readback requirements, and no-send boundaries.",
  },
];
