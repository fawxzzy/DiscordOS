...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "Host-control state is now visible in a dashboard; the next value is proving one guarded live apply and readback path against that dashboard.",
    does: "Runs a guarded host-control storage apply, reloads persisted session state, and confirms the dashboard reflects the new queue/vote/close state.",
  },
  {
    ...
    why: "Provider preview buttons are modeled; the next risk is whether a selected provider track becomes a persisted queue item safely.",
    does: "Executes one guarded provider-selection queue write with metadata only, then verifies no playback or slash-command path was admitted.",
  },
  {
    ...
    why: "Audit attention now has critical routing; the next value is proving delivery admission without leaking raw actor or token data.",
    does: "Runs a controlled alert-delivery canary for button-route audit attention through the existing alert channel policy.",
  },
  {
    ...
    why: "Delivery policy is visible; the next step is a gate that decides which non-testing surfaces can receive bot responses.",
    does: "Classifies Music Sesh response targets by channel policy, mention safety, and readback requirements before any broader send path opens.",
  },
  {
    ...
    why: "Auto-repair now has a canary; the next value is proving one live repair and readback cycle without touching unrelated reactions.",
    does: "Applies guarded repair to one admitted board card, removes legacy/opposite reactions, and confirms live readback matches committed state.",
  },
];
