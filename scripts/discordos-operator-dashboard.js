...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "Trend alert routing now maps repeated host-control conflict or drift into the operator attention path; the next value is a guarded delivery canary for that routed alert.",
    does: "Exercises the routed host-control trend alert through delivery admission without sending messages or controlling playback.",
  },
  {
    ...
    why: "The provider queue interaction canary now signs a metadata-only button plan; the next value is an admission gate before any live button execution.",
    does: "Checks signed interaction identity, metadata-only queue behavior, and no-provider/no-playback boundaries before live execution is admitted.",
  },
  {
    ...
    why: "Acknowledgement persistence now previews handled state with redaction; the next value is reading that state back so closed alerts remain visibly closed.",
    does: "Confirms persisted acknowledgement state, handled timestamps, and redacted actor/token fields through a bounded readback.",
  },
  {
    ...
    why: "Rate-limit enforcement now decides allow/throttle before delivery; the next value is showing those decisions to operators without leaking user content.",
    does: "Summarizes per-channel allow/throttle outcomes, remaining response budget, and mention-safety status for operator review.",
  },
  {
    ...
    why: "Scheduler rollup alerts now suppress aligned-card noise and route only real repair attention; the next value is a guarded delivery canary for that alert path.",
    does: "Exercises repair scheduler alert routing through delivery admission while preserving custom-reaction guards and readback requirements.",
  },
];
