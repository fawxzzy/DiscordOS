...
const HIGHEST_VALUE_CATEGORIES = [
  {
    id: "music_sesh_queue_conflict_and_host_controls",
    label: "Music Sesh queue conflict and host controls",
    command: "npm run ops:discordos:music-sesh-session-lifecycle-buttons",
    why: "The session lifecycle buttons are covered; the next risk is two users or a host action racing on lock, close, vote, or queue state.",
    does: "Defines conflict handling for lock, close, vote, and queue actions so concurrent channel interactions resolve predictably without slash commands.",
  },
  {
    ...
    why: "The metadata adapter contract is now explicit; the next useful step is a live read-only canary that proves provider search output shape before playback exists.",
    does: "Runs a guarded provider metadata/search canary, normalizes returned track fields, and still blocks play, pause, skip, and stop controls.",
  },
  {
    ...
    why: "Button route audit events now have a persistence contract; readback makes those events searchable and useful after execution.",
    does: "Queries sanitized audit rows by interaction/session and proves the stored record excludes signatures, tokens, and raw user identifiers.",
  },
  {
    id: "music_sesh_user_response_delivery_guard",
    label: "Music Sesh user response delivery guard",
    command: "npm run ops:discordos:chat-message-live-ingest -- --content \"computa music status\"",
    why: "Status response text is generated; delivery rules should be explicit before the bot posts user-facing messages in live channels.",
    does: "Proves when responses may be delivered in-channel, keeps allowed mentions disabled, and preserves the current no-slash interaction doctrine.",
  },
  {
    ...
    why: "Live reaction reconciliation exists; the next board value is continuous drift detection across every Music Sesh feature card.",
    does: "Monitors forum reaction state against board state and reports mismatched cards without mutating public channels unless explicitly admitted.",
  },
];
