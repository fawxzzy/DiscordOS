...
const HIGHEST_VALUE_CATEGORIES = [
  {
    ...
    why: "The host-control history can now classify repeated conflict and readback drift trends; the next value is routing watch-level trends to the operator attention path.",
    does: "Maps clear/watch host-control trends into bounded operator alert decisions without sending messages or controlling playback.",
  },
  {
    ...
    why: "The provider queue interaction readback now maps a button custom id to queue metadata only; the next value is a guarded live canary for that interaction.",
    does: "Exercises one signed provider queue button interaction under guard and confirms metadata-only queue behavior through readback.",
  },
  {
    ...
    why: "The acknowledgement flow now has no-slash metadata and redaction; the next value is persisting handled state so closed alerts stay closed.",
    does: "Persists bounded acknowledgement state for handled button-route audit alerts with actor/token redaction preserved.",
  },
  {
    ...
    why: "The per-channel rate-limit policy now exists as a safe read model; the next value is enforcing it before user-facing responses send.",
    does: "Applies the rate-limit decision ahead of guarded Music Sesh response delivery while keeping mention safety and no slash entrypoints.",
  },
  {
    ...
    why: "The scheduler now has an observability rollup; the next value is alerting only when repair attempts or readback gaps need operator attention.",
    does: "Converts scheduler repair rollups into bounded alert decisions while skipping aligned-card noise.",
  },
];
