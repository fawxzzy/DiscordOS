...
} = require("./discordos-music-provider-queue-interaction-admission-history-alert-delivery-history-alert-delivery-readback");

function parseArgs(args) {
  return readbackInternals.parseArgs(args);
}

function buildProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard(readbackResult) {
...
function validateProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard({ readbackResult, dashboard }) {
...
async function buildMusicProviderQueueInteractionAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard(input = {}) {
  const readbackResult = await readbackInternals.buildMusicProviderQueueInteractionAdmissionHistoryAlertDeliveryHistoryAlertDeliveryReadback(input);
  const dashboard = buildProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard(readbackResult);
  const reasonCodes = validateProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard({ readbackResult, dashboard });
  const result = {
    ...
    executesStorageWrite: false,
    slashCommandsAdmitted: false,
    status: reasonCodes.length === 0 ? "music_provider_queue_interaction_admission_history_alert_delivery_history_alert_delivery_dashboard_ready" : "blocked",
  ...
  return {
    ...
    event: {
      type: result.ok
        ? "discordos.music_provider.queue_interaction_admission_history_alert_delivery_history_alert_delivery_dashboard_ready"
        : "discordos.music_provider.queue_interaction_admission_history_alert_delivery_history_alert_delivery_dashboard_blocked",
      severity: result.ok ? "info" : "warning",
      subject: "discordos.music_provider.queue_interaction_admission_history_alert_delivery_history_alert_delivery_dashboard",
...
function renderMarkdown(result) {
  return [
    "# DiscordOS Music Provider Queue Interaction Admission History Alert Delivery History Alert Delivery Dashboard",
    "",
...
async function main() {
  try {
    ...
    const result = await buildMusicProviderQueueInteractionAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard(options);
...
module.exports = {
  _internals: {
    parseArgs,
    buildProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard,
    validateProviderAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard,
    buildMusicProviderQueueInteractionAdmissionHistoryAlertDeliveryHistoryAlertDeliveryDashboard,
    renderMarkdown,
  },
};
