...
const DEFAULT_DOCS_FILE = path.resolve(process.cwd(), "docs", "contracts", "discordos-board-card-schema-admission-v0.md");
...
const REQUIRED_SOURCE_TOKENS = [
  "DiscordOSBoardCardSchemaAdmissionStatus",
  "DiscordOSBoardCardSchemaAdmissionPlan",
  "planning_ready",
...
function classifyRuntimeFreeSource(sourceText) {
  ...
  return {
    ok: banned.length === 0,
    banned,
    reasonCodes: banned.length === 0 ? [] : ["board_card_schema_admission_runtime_token_present"],
  };
}

function classifyBoardCardSchemaAdmissionEvent(result) {
  return {
    type: result.ok
      ? "discordos.board_card.schema_admission_ready"
      : "discordos.board_card.schema_admission_blocked",
    severity: result.ok ? "info" : "warning",
    subject: "discordos.board_card.schema_admission",
...
async function buildBoardCardSchemaAdmissionStatus({
  docsFile = DEFAULT_DOCS_FILE,
  sourceFile = DEFAULT_SOURCE_FILE,
  fsImpl = fs,
} = {}) {
  const docsRead = await readText(docsFile, "board_card_schema_admission_docs_missing", fsImpl);
  const sourceRead = await readText(sourceFile, "board_card_schema_admission_source_missing", fsImpl);
  const docs = docsRead.ok
    ? classifyRequiredText(docsRead.text, REQUIRED_DOC_ANCHORS, "board_card_schema_admission_docs_anchor_missing")
    : {
      ok: false,
      present: [],
  ...
  const source = sourceRead.ok
    ? classifyRequiredText(sourceRead.text, REQUIRED_SOURCE_TOKENS, "board_card_schema_admission_source_token_missing")
    : {
      ok: false,
      present: [],
  ...
  return {
    ...result,
    event: classifyBoardCardSchemaAdmissionEvent(result),
  };
}

function renderMarkdown(result) {
  return [
    "# DiscordOS Board Card Schema Admission Status",
    "",
...
async function main() {
  try {
    ...
    const result = await buildBoardCardSchemaAdmissionStatus(options);
...
module.exports = {
  _internals: {
    ...
    classifyBoardCardSchemaAdmissionEvent,
    buildBoardCardSchemaAdmissionStatus,
    renderMarkdown,
  },
};
