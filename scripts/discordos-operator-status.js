...
async function buildDiscordOSOperatorStatus({
  ...
  const status = {
    ...
    messageCommandPoll: {
      ...
      healthySource: messageCommandPollStatus.healthySource,
      workflowName: messageCommandPollStatus.workflowName,
      workflowState: messageCommandPollStatus.workflowState,
      workerWorkflowName: messageCommandPollStatus.workerWorkflowName,
      workerWorkflowState: messageCommandPollStatus.workerWorkflowState,
      ...
      latestWorkerRunAgeMinutes: messageCommandPollStatus.latestWorkerRun?.ageMinutes ?? null,
      latestWorkerRunStatus: messageCommandPollStatus.latestWorkerRun?.status || null,
      latestWorkerRunConclusion: messageCommandPollStatus.latestWorkerRun?.conclusion || null,
      latestWorkerRunUrl: messageCommandPollStatus.latestWorkerRun?.url || null,
...
function renderMarkdown(status) {
  const lines = [
    ...
    `- healthy source: \`${status.messageCommandPoll.healthySource || "none"}\``,
    `- workflow: \`${status.messageCommandPoll.workflowName}\``,
    `- workflow state: \`${status.messageCommandPoll.workflowState}\``,
    `- worker workflow: \`${status.messageCommandPoll.workerWorkflowName || "none"}\``,
    `- worker workflow state: \`${status.messageCommandPoll.workerWorkflowState || "none"}\``,
    ...
    `- latest worker run age minutes: \`${status.messageCommandPoll.latestWorkerRunAgeMinutes ?? "none"}\``,
    `- latest worker run status: \`${status.messageCommandPoll.latestWorkerRunStatus || "none"}\``,
    `- latest worker run conclusion: \`${status.messageCommandPoll.latestWorkerRunConclusion || "none"}\``,
    `- latest worker run url: \`${status.messageCommandPoll.latestWorkerRunUrl || "none"}\``,
    ...
    `- status: \`${status.atlasHealth.status || "unknown"}\``,
    `- event type: \`${status.atlasHealth.eventType}\``,
    `- watch status: \`${status.atlasHealth.watchStatus || "unknown"}\``,
