...
- eight project forums are admitted, including Socials OS; the governed registry denominator is 13. Socials resolves its exact channel ID during first provision.
- `npm run ops:discordos:project-board-forum-provision:json` retains the guarded, idempotent provision/readback path for those eight forums.
...
- `npm run ops:discordos:canonical-board-migration:json -- --snapshot-output <runtime-snapshot.json> --output <runtime-plan.json>` is the dry-run-default serialized 13-board plan. Apply additionally requires `DISCORDOS_CANONICAL_BOARD_MIGRATION=enabled`, `--allow-migration`, and `--apply`.
...
- `docs/contracts/discordos-forum-profile-normalization-v1.md`
  - defines the canonical 13-board title/profile/tag contract, retained-history disposition, serialized guarded migration, Socials OS admission/seed, and exact readback
