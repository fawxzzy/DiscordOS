...
test("mazer feedback board reads committed cards", async () => {
  ...
  assert.equal(result.openCardCount, 35);
  assert.equal(result.readyCardCount, 1);
  assert.equal(result.completedCardCount, 9);
  ...
  assert.equal(result.nextCard.id, "mazer-crisp-cyber-arcade-graphics");
  assert.equal(result.cards.find((card) => card.id === "mazer-crisp-cyber-arcade-graphics").state, "ready");
  ...
  assert.deepEqual(result.planning, {
    ok: true,
    activeCardId: "mazer-crisp-cyber-arcade-graphics",
    epicCount: 9,
    mappedCardCount: 64,
    dependencyCount: 34,
