...
const { _internals } = require("../scripts/discordos-music-provider-queue-interaction-admission-history-alert-delivery-history");

test("provider queue admission alert delivery history tracks metadata-only dashboard states", async () => {
  const result = await _internals.buildMusicProviderQueueInteractionAdmissionHistoryAlertDeliveryHistory();
...
test("provider queue admission alert delivery history rejects metadata drift", () => {
  const reasonCodes = _internals.validateProviderAdmissionHistoryAlertDeliveryHistory({
    dashboardResult: {
      reasonCodes: [],
      sendsMessages: false,
  ...
  assert(reasonCodes.includes("provider_queue_interaction_admission_history_alert_delivery_history_record_invalid"));
});
