...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 124);
  assert.equal(dashboard.productRuntime.availableCount, 124);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_rollup_dashboard_history");
  assert.match(dashboard.highestValueCategories[0].why, /rollup dashboard now makes/);
  assert.match(dashboard.highestValueCategories[0].does, /Tracks recent host-control/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 124);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alert_delivery_readback"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery rollup dashboard history`"));
  assert(rendered.includes("why: The rollup dashboard now makes"));
  assert(rendered.includes("does: Tracks recent host-control rollup dashboard states"));
