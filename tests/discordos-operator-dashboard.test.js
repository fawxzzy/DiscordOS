...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 129);
  assert.equal(dashboard.productRuntime.availableCount, 129);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_rollup_dashboard_history_alerting");
  assert.match(dashboard.highestValueCategories[0].why, /dashboard history now tracks/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 129);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alert_delivery_dashboard"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery rollup dashboard history alerting`"));
  assert(rendered.includes("why: The rollup dashboard history now tracks"));
  assert(rendered.includes("does: Classifies host-control rollup dashboard history"));
