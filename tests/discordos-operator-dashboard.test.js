...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 139);
  assert.equal(dashboard.productRuntime.availableCount, 139);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_rollup_dashboard_history_alert_delivery_readback");
  assert.match(dashboard.highestValueCategories[0].why, /canary now proves no-send admission/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 139);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alert_delivery_history_alerting"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery rollup dashboard history alert delivery readback`"));
  assert(rendered.includes("why: The host-control rollup dashboard history alert delivery canary now proves"));
  assert(rendered.includes("does: Reads host-control rollup dashboard history alert delivery"));
