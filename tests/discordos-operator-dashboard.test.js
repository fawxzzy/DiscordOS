...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 114);
  assert.equal(dashboard.productRuntime.availableCount, 114);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_history_rollup");
  assert.match(dashboard.highestValueCategories[0].why, /delivery history/);
  assert.match(dashboard.highestValueCategories[0].does, /Summarizes recent/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 114);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alerting"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery history rollup`"));
  assert(rendered.includes("why: The delivery history now captures"));
  assert(rendered.includes("does: Summarizes recent host-control alert delivery history"));
