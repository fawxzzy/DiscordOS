...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 109);
  assert.equal(dashboard.productRuntime.availableCount, 109);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_history");
  assert.match(dashboard.highestValueCategories[0].why, /delivery dashboard/);
  assert.match(dashboard.highestValueCategories[0].does, /Tracks recent/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 109);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery history`"));
  assert(rendered.includes("why: The delivery dashboard now makes"));
  assert(rendered.includes("does: Tracks recent host-control trend alert delivery dashboard states"));
