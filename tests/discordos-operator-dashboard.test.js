...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 169);
  assert.equal(dashboard.productRuntime.availableCount, 169);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_rollup_dashboard_history_alert_delivery_history_alert_delivery_dashboard");
  assert.match(dashboard.highestValueCategories[0].why, /readback now preserves guarded admission/);
  assert.match(dashboard.highestValueCategories[0].does, /Summarizes host-control/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 169);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alert_delivery_history_alert_delivery_history_alert_delivery_canary"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery rollup dashboard history alert delivery history alert delivery dashboard`"));
  assert(rendered.includes("why: The host-control alert delivery readback now preserves guarded admission"));
  assert(rendered.includes("does: Summarizes host-control alert delivery readback decisions"));
