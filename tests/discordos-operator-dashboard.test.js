...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 119);
  assert.equal(dashboard.productRuntime.availableCount, 119);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_trend_alert_delivery_history_rollup_dashboard");
  assert.match(dashboard.highestValueCategories[0].why, /rollup now summarizes/);
  assert.match(dashboard.highestValueCategories[0].does, /Shows host-control/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 119);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_interaction_admission_history_alert_delivery_canary"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control trend alert delivery history rollup dashboard`"));
  assert(rendered.includes("why: The rollup now summarizes"));
  assert(rendered.includes("does: Shows host-control trend alert delivery rollups"));
