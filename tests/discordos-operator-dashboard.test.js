...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 74);
  assert.equal(dashboard.productRuntime.availableCount, 74);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_live_apply_dashboard_rollup");
  assert.match(dashboard.highestValueCategories[0].why, /live apply reconciliation can prove one guarded write/);
  assert.match(dashboard.highestValueCategories[0].does, /Summarizes recent live host-control apply attempts/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 74);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_queue_selection_user_button_surface"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control live apply dashboard rollup`"));
  assert(rendered.includes("why: Host-control live apply reconciliation can prove one guarded write"));
  assert(rendered.includes("does: Summarizes recent live host-control apply attempts"));
