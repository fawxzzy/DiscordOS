...
test("operator dashboard summarizes next-work result into command hint", async () => {
  ...
  assert.equal(dashboard.productRuntime.surfaceCount, 69);
  assert.equal(dashboard.productRuntime.availableCount, 69);
  assert.equal(dashboard.highestValueCategories.length, 5);
  assert.equal(dashboard.highestValueCategories[0].id, "music_sesh_host_control_live_apply_reconciliation");
  assert.match(dashboard.highestValueCategories[0].why, /Host-control state is now visible/);
  assert.match(dashboard.highestValueCategories[0].does, /guarded host-control storage apply/);
...
test("operator dashboard exposes product runtime command tiles", () => {
  ...
  assert.equal(panel.surfaceCount, 69);
...
test("operator dashboard exposes ranked highest-value categories", () => {
  ...
  assert(categories.some((category) => category.id === "music_provider_selection_to_queue_live_canary"));
...
test("operator dashboard renders compact markdown without target values", () => {
  ...
  assert(rendered.includes("category 1: `Music Sesh host control live apply reconciliation`"));
  assert(rendered.includes("why: Host-control state is now visible"));
  assert(rendered.includes("does: Runs a guarded host-control storage apply"));
