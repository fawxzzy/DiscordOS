...
- when no trusted resume receipt exists, a plan-created destination has the exact required archive postimage `archived:false` and `locked:false`; either flag being true blocks both runtime preflight and terminal reconciliation with `completed_card_destination_archive_postimage_drift`;
...
- every blocked completed-transfer receipt that created or adopted a destination persists its exact stable thread ID and the destination archive/lock state that must survive the operation. This is `false/false` for a newly created destination and the captured live preimage for an existing destination; absent such a trusted receipt, `false/false` remains the plan-bound created-destination expectation rather than disabling archive-state validation. A failed create records no invented destination state. A later apply may use that prior exact plan/evidence-bound blocked receipt through `--resume-receipt`; the executor first binds the entire file to the independently trusted exact-byte digest, then binds its plan, evidence, card, operation, destination ID, and state before allowing the partial destination to resume. Tampering either archive/lock direction fails before live preflight and at zero writes;
- when a trusted resume state is already complete but a later operation blocks, the newer top-level receipt carries that exact card, destination thread, archive, and lock authority on the completed operation row. Any trusted state not otherwise represented is emitted as a no-write `resume_state_preserved` row. Extraction binds those carried fields to the operation and exact receipt bytes, cross-checks any nested transfer state, and rejects destination-ID/archive/lock tampering before scans or writers; this preservation never supplies authority when no trusted receipt exists;
