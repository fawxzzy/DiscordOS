...
- curated `#updates` publication now has a DiscordOS-owned dry-run-first command with green embed formatting, disabled mentions, guarded apply checks, status inspection, receipt audit rollups, a combined operator status bundle, a next-work recommender, and pulled-env normalization for Discord target values
