...
The exact-head review repair adds fail-closed guarantees: recomputed modified plans remain unauthorized; archived destinations and journal history are paginated to bounded exhaustion; unreadable destination starters block creation; strict transfer execution binds the reread source title and all destination/body/journal derivation to the reviewed plan title; pristine source admission requires explicit `archived:false` while allowing Discord to omit unlocked `locked`; plan-backed destination body, journal, source reciprocal body, tags, state, reaction, archive, and lock are exact deterministic postimages; without a trusted resume receipt, the plan-created destination expectation remains exactly open/unlocked (`false/false`) and archived or locked drift blocks both preflight and terminal reconciliation; exact archived destinations replay without reopening only when a trusted prior-state receipt authorizes that state, repair-needed archived destinations restore their original archive/lock state, and journal failure blocks later tag/reaction/source mutation because required success-reaction reconciliation is deferred until the journal succeeds; the exact reciprocal-link-written/source-open intermediate can resume only the remaining archive+lock transition; every blocked transfer receipt with an actual destination durably records the exact destination thread and required archive/lock state, including open/unlocked for a newly created destination and the captured archived/locked preimage when a reopen outcome is unknown, while failed creation records no invented state; trusted destination ID/archive/lock authority is also carried on earlier completed transfer rows whenever a later operation blocks, so a newer receipt cannot erase a legitimate non-default preimage, and any otherwise unrepresented authority is retained in a no-write preservation row; apply-only resume requires an independently trusted exact-byte receipt digest before binding the plan/evidence/card/operation/destination/state fields, rejects destination-ID/archive/lock tampering before scans or writers, and retains that authority across repeated later barriers; rejected Discord request promises—including tag, order, destination reopen, restore, and final reconciliation reads—become blocked receipts with explicit read-rejected or write-outcome-unknown reasons rather than escaping before receipt persistence; an ambiguously rejected restore is never automatically replayed, while a definitive HTTP failure retains the bounded retry; every tag and order write gets an immediate exact compare-before-write guard; terminal reconciliation re-runs every plan-bound completed-transfer inspection and requires the exact source and destination body/link/journal/reaction/tag/archive/lock postimages in addition to scanner aggregates; applied tag IDs use duplicate-sensitive set comparison; full guild-channel readback proves unrelated channels remain invariant during the bounded 13-board reorder; standalone stable-ID replay remains no-write when no reviewed source preimage is supplied; and every successful Discord write is counted even when a later step blocks.

## Verification evidence

- focused repair suite: `27/27 passed`
- focused repair + completed-transfer cluster: `60/60 passed`
...
## Sibling evidence divergence

Draft PR `#102` at `b8b77d1adfcdfeae805c009db61994858549fec8` is a sibling branch from the same base `38e881d4265206c2148caaeb6a6a457e3e5bcda8`. It touches only `config/discordos-mazer-feedback-board.json` and `docs/ops/discordos-mazer-pr80-account-browser-parity-2026-07-16.md`; neither path overlaps this packet. PR `#102` was not absorbed, rewritten, closed, merged, or modified here.

PR `#102`'s receipt reports that its three exact Mazer production journals observed a live registry scan with `245` current identities, while this packet's admitted evidence and plan remain frozen at `243/243`. The Mazer receipt says no card was created and no broad sync ran; this packet does not infer the cause of the two-row difference. It classifies the observation as changed post-admission live evidence.

The frozen packet remains fail-closed:

- applying the frozen plan against a `245` live scan stops before every writer phase with `current_card_denominator_mismatch` and, when the healthy count is also `245`, `current_healthy_card_denominator_mismatch`;
- substituting refreshed evidence under the frozen plan fails `admitted_evidence_digest_mismatch`, `plan_evidence_raw_digest_mismatch`, and `plan_evidence_canonical_digest_mismatch` before live preflight;
- generating from `245` under this fixed-event contract stops with `admitted_current_card_denominator_mismatch` rather than silently changing the plan.

The next live packet must recompute producer-backed current evidence. It may proceed only after a separately justified replacement contract/plan admits the resulting denominator and full drift set; otherwise it must remain blocked on the stable reason codes above. This code-only packet does not refresh the admitted scan or widen the frozen plan.
